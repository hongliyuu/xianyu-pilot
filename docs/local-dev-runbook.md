...
- Web 开发页：`http://127.0.0.1:12400/#/login`
- API：`http://127.0.0.1:12401`
- API 存活检查：`http://127.0.0.1:12401/health`
- API 就绪检查：`http://127.0.0.1:12401/health/ready`（数据库、迁移、Redis、消息自动化与 Token 刷新器均须为 `ok`）
- crawler：`http://127.0.0.1:12402`
- crawler 就绪检查：`http://127.0.0.1:12402/ready`（配置与 Playwright Chromium 均须可用）
...
首次本地开发先运行准备脚本，再运行交互式初始化脚本。初始化脚本从
`.env.development.example` 创建被 Git 忽略的 `.env`，仅收集本地管理员密码并生成
`ADMIN_PASSWORD_HASH`；它不会打印或提交该值。初始化完成后，请自行编辑 `.env` 中的
MySQL 和 Redis 连接信息，填写实际存在且可连接的开发数据库；不要把生产 `.env` 用于本地调试。

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\bootstrap-local-dev.ps1
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\init-local-dev.ps1
notepad .env
.\start-local.bat
```

`bootstrap-local-dev.ps1` 会创建 `apps/api/.venv`、安装锁定的 API 依赖、安装 Web/Crawler
依赖，并显式安装 Crawler 所需的 Playwright Chromium。启动、状态、停止和本地验证脚本统一使用该虚拟环境，
不再回退到全局 Python。API 本地环境要求 Python `>=3.11`；生产运行时仍使用 Python 3.11。
准备脚本会优先使用当前 `python`，仅当它不满足要求时才尝试 Windows 的 `py -3.11`。

如需重新创建 `.env`，显式传入 `-Force`。该操作会覆盖整个 `.env` 并重新生成管理员密码哈希，之后必须重新填写
MySQL 和 Redis 连接信息：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\init-local-dev.ps1 -Force
notepad .env
```

如需仅重置本地管理员密码并保留已有的 MySQL、Redis 配置，使用：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\init-local-dev.ps1 -ResetAdminPassword
```

重置后需重启本地 API，新的 `.env` 密码哈希才会生效。
...
Pop-Location
.\apps\api\.venv\Scripts\python.exe .\apps\api\run.py
```
...
手动启动时可用 `.\.venv\Scripts\python.exe -m app.worker --check` 验证心跳；受管脚本会为本地栈设置独立的心跳文件，避免与其他项目实例混淆。
...
```powershell
$env:VITE_API_PROXY_TARGET='http://127.0.0.1:12401'
...
- `http://127.0.0.1:12401/health/ready` 返回 HTTP 200、`status=ready`，且全部 `components` 为 `ok`
- `http://127.0.0.1:12402/ready` 返回 HTTP 200、`status=ready`
- `http://127.0.0.1:12400/#/login` 可访问
...
- 登录后 `GET /api/system/runtime-status` 返回：
  - `crawlerBaseUrl=http://127.0.0.1:12402`
...
- `POST http://127.0.0.1:12401/api/auth/login` 可成功登录固定管理员账号
- `POST http://127.0.0.1:12401/api/qrlogin/generate` 可成功生成二维码登录会话
...
主站扫码不经过 Crawler；Crawler 的 `12402` 就绪状态只影响滑块求解。确认 API 与
